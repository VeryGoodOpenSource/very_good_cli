  ...
  group(TestCLIRunner, () {
    ...
    group('.test', () {
        ...
        group('when the runner type is $name', () {
          test(
            ...
            () async {
              ...
              await ProcessSignalOverrides.runZoned(() async {
          ...
          test('throws when pubspec not found', () async {
            await expectLater(
              () =>
                  Flutter.test(logger: logger, cwd: Directory.systemTemp.path),
      ...
      test('runs tests (error)', () async {
        ...
        expect(stderrLogs.first, equals('\x1B[2K\r$exception'));
        expect(stderrLogs[1], startsWith('\x1B[2K\r'));
      });
      ...
      group('timeout parameter', () {
        test('completes with unavailable when timeout fires', () async {
          final tempDirectory = Directory.systemTemp.createTempSync();
          addTearDown(() => tempDirectory.deleteSync(recursive: true));
          ...
          // Stream that never emits ExitTestEvent — simulates a hung test.
          final controller = StreamController<TestEvent>();
          addTearDown(controller.close);

          final result = await TestCLIRunner.test(
            testType: TestRunType.flutter,
            cwd: tempDirectory.path,
            logger: logger,
            stdout: stdoutLogs.add,
          ...
          expect(
            stderrLogs,
            contains(contains('timed out after')),
          );
        });

        test('cancels timer on normal exit', () async {
          final tempDirectory = Directory.systemTemp.createTempSync();
          addTearDown(() => tempDirectory.deleteSync(recursive: true));
          ...
          await expectLater(
            TestCLIRunner.test(
              ...
              stdout: stdoutLogs.add,
              stderr: stderrLogs.add,
              // Long enough that normal exit wins.
        ...
        test('cancels timer on SIGINT', () async {
          final streamController = StreamController<ProcessSignal>();
          await ProcessSignalOverrides.runZoned(() async {
            final tempDirectory = Directory.systemTemp.createTempSync();
            addTearDown(() => tempDirectory.deleteSync(recursive: true));
            ...
            // Stream that never resolves — SIGINT should win.
            final controller = StreamController<TestEvent>();
            addTearDown(controller.close);
      ...
      group('collectCoverageFrom parameter', () {
        ...
        test('respects collectCoverageFrom imports mode (default)', () async {
          final tempDirectory = Directory.systemTemp.createTempSync();
          addTearDown(() => tempDirectory.deleteSync(recursive: true));
