...
void main() {
  group('test', () {
    ...
    test('completes normally without running tests '
        'when Flutter is not installed', () async {
    ...
    group('arguments', () {
      ...
      test('completes normally --dart-define-from-file', () async {
        when<dynamic>(() => argResults['dart-define-from-file'])
      ...
      test(
        ...
        () async {
          ...
          verify(() => logger.err(any(that: contains('--recursive'))))
              .called(1);
        },
      );

      test('allows --recursive when rest arguements are all options', () async {
        when<dynamic>(() => argResults['recursive']).thenReturn(true);
    ...
    group('sharding', () {
      ...
      test('fails when sharding with optimization disabled', () async {
        ...
        verifyNever(() => flutterTest(shardIndex: 1, totalShards: 3));
      });
      ...
      test('fails when sharding with --update-goldens', () async {
        withShards('1', '3');
        when<dynamic>(() => argResults['update-goldens']).thenReturn(true);
    ...
    group('very_good.yaml configuration', () {
      ...
      test('applies config value when arg was not parsed', () {
        ...
        expect(options.rawMinCoverage, isNull);
