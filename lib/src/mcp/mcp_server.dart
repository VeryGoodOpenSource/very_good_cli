...
final class VeryGoodMCPServer extends MCPServer with ToolsSupport {
  ...
  VeryGoodMCPServer({
    ...
    Logger? logger,
       ...
       super.fromStreamChannel(
         ...
         instructions: 'A Very Good CLI MCP server that provides tools '
        ...
        inputSchema: ObjectSchema(
          properties: {
            ...
            'platforms': StringSchema(
              description: 'Comma-separated platforms (flutter_plugin only). '
    ...
    registerTool(
      Tool(
        ...
        inputSchema: ObjectSchema(
          properties: {
            'directory': StringSchema(description: 'Project directory'),
            'licenses': BooleanSchema(description: 'Check licenses'),
          },
        ),
      ),
  ...
  Future<CallToolResult> _handleCreate(CallToolRequest request) async {
    try {
      ...
      if (args['output_directory'] != null) {
        cliArgs.addAll(['--output-dir', args['output_directory']! as String]);
      }
  ...
  Future<CallToolResult> _handlePackagesCheck(CallToolRequest request) async {
    try {
      ...
      final cliArgs = <String>['packages', 'check'];
      
      if (args['licenses'] == true) {
        cliArgs.add('licenses');
      }
      if (args['directory'] != null) {
        cliArgs.add(args['directory']! as String);
      }
  ...
  Future<int> _runCommand(List<String> args) async {
    try {
      ...
      final runner = CommandRunner<int>('very_good', 'CLI')
