...
void main() {
  group('LoggerX', () {
    ...
    group('wrap', () {
      ...
      test('''defaults to `fallbackStdoutTerminalColumns` when there is no terminal''', () async {
        await IOOverrides.runZoned(stdout: () => stdout, () async {
          ...
          verifyInOrder([
            () => logger.info(any(that: equals('$longWord '))),
            () => logger.info(any(that: equals('$shortWord '))),
          ]);
        });
      });
      ...
      test('''throws when an unknown exception occurs when reading `stdout.terminalColumns`''', () async {
        await IOOverrides.runZoned(stdout: () => stdout, () async {
